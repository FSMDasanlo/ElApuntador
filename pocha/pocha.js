document.addEventListener("DOMContentLoaded", () => {
  ...
  function inicializarTabla(tabla, cuerpoTabla, rondas, esTablaAscendente) {
    ...
    jugadores.forEach(nombre => {
      ...
      // --- ¡NUEVO! Creamos spans para la insignia de ranking y el nombre ---
  ...
  function actualizarRanking() {
    // 1. Obtenemos los datos y los ordenamos (puntuación más alta primero)
    ...
    // 2. Creamos un mapa de ranking (nombre -> posición)
    ...
    datos.forEach((jugador, index) => {
      ...
      rankMap.set(jugador.nombre, currentRank);
      lastScore = jugador.puntos;
    });

    // 3. Actualizamos las insignias en AMBAS tablas
    const todasLasFilas = document.querySelectorAll('#tabla-puntos-ascendente tbody tr, #tabla-puntos-descendente tbody tr');
    todasLasFilas.forEach(row => {
      const nombre = row.getAttribute('data-jugador');
      const rank = rankMap.get(nombre);
      const spanRank = row.querySelector('.rank-prefix');

      if (spanRank) {
        spanRank.textContent = rank ? `${rank}º` : '';
        // Limpiamos clases de color antiguas
        spanRank.classList.remove('rank-1', 'rank-2', 'rank-3');
        // Añadimos la nueva clase de color si está en el podio
        if (rank > 0 && rank <= 3) {
          spanRank.classList.add(`rank-${rank}`);
        }
      }
    });
  }
...
window.obtenerRankingParaVoz = function() {
  ...
  const datos = Array.from(filas).map(row => {
    const nombre = row.getAttribute('data-jugador');
    const puntos = parseInt(row.querySelector(".total").textContent || 0);
    return { nombre, puntos };
  });
  // En la Pocha, más puntos es mejor.
  datos.sort((a, b) => b.puntos - a.puntos);
  return datos;
};
